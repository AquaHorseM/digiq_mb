...
parser.add_argument("--num_actions", type=str, default="", help="Number of actions to visualize")
...
print("List of successful trajectories:", [i for i in range(len(data)) if data[i][-1]['reward'] > 0])
...
def display_images(trajectory_id, step_id):
    trajectory_id = int(trajectory_id)
    if trajectory_id < 0 or trajectory_id >= len(data):
        return ["Invalid index. Please enter a valid index."] + [None]
    ...
    for action_id in range(num_actions):
        ...
        step_data['action'] = step_data['action_list'].split("<split>")[action_id]
        step_data['q_rep_out'] = step_data['q_rep_out_list'][action_id]
        ...
        if args.vis_state_values == "True":
            out_tensor = torch.from_numpy(step_data['q_rep_out']).to(agent.device)
            ...
            v, q = round(v.cpu().detach().numpy().item(), 2), round(q.cpu().detach().numpy().item(), 5)
        ...
        if args.parse_action == "True":
            action_dict = parse_action(step_data['action'])
    ...
    if images:
        ...
        return [task] + ["No images to display."]


with gr.Blocks() as interface:
    gr.Markdown("## Action Viewer (Step-level, Multiple Actions)")
    
    with gr.Row():
        with gr.Column():
            trajectory_id = gr.Number(label="Trajectory Index")
            step_id = gr.Number(label="Step Index")
        with gr.Column():
    ...
    with gr.Row():
        with gr.Column():
            image_outputs.append(gr.Image(label=f"Q-V Value Visualization (DigiQ)"))
    
    trajectory_id.change(display_images, inputs=[trajectory_id, step_id], outputs=[task_description_output] + image_outputs)
    step_id.change(display_images, inputs=[trajectory_id, step_id], outputs=[task_description_output] + image_outputs)

print("Launching the Multi-action Step-level Viewer...")
