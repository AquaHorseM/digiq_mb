...
class ValueModel_Trainer:
    def __init__(self, accelerator:Accelerator=None, load_path:str=None, save_path:str=None, epoch:int=None, val_interval:int=None,
                 state_dim:int=None, goal_dim:int=None, embed_dim:int=None, num_attn_layers:int=None, num_heads:int=None):
        self.accelerator = accelerator
        self.device = self.accelerator.device
        
        self.value_model = ValueModel(state_dim=state_dim, goal_dim=goal_dim, embed_dim=embed_dim, num_attn_layers=num_attn_layers, num_heads=num_heads, device=self.device)
    ...
    def loss(self, batch):
        states, value = batch['state'], batch['value']
        value_pre = self.value_model.forward(states)
        loss = F.mse_loss(value, value_pre)
